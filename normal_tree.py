...
if __name__ == "__main__":
    ...
    parser.add_argument('--data', help='choose dataset version', choices=['1st', '2nd', '3rd'], default='3rd')
    args = parser.parse_args()

    option_dir_list = ['normal_tree','normal_tree_normalize','normal_tree_weight']

    max_depth = args.depth
    option = args.weight_option

    # 学習データの読み込み
    output = Path(f'./results/{option_dir_list[option]}/{args.data}/{args.mode}/')
    output.mkdir(exist_ok=True, parents=True)
    data, label, feature_names, label_names, annotation = utils.readCSV_svs(args.data, args.mode)
    print(label_names)
    ...
    for depth in np.array(range(max_depth))+1:
        ...
        utils.makeCSV2(tree.pred, outputdir, 'Ptable_normal.csv')
        utils.makeCSV2(tree.leaf_hist, outputdir, 'hist_normal.csv')
        utils.makeCSV2(tree.leaf_data, outputdir, 'leaf_list_normal.csv')
