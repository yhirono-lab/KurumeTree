...
if __name__ == '__main__':
    ...
    # 学習データの読み込み
    output = Path(f'./results/kurume_tree/{args.data}/{args.mode}')
    output.mkdir(exist_ok=True, parents=True)
    data, label, feature_names, label_names, annotation = utils.readCSV_svs(args.data, args.mode, unu_flag=False)
    print(data.shape)
    print(label_names)
